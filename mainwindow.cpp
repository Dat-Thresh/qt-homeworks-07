    ...
    , ui(new Ui::MainWindow)
{
    ...
    custom_pl_chart = new QCustomPlot(window);

    custom_pl_chart->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    lay->addWidget(custom_pl_chart);
    window->setLayout(lay);

    custom_pl_chart->setInteraction(QCP::iRangeZoom, true);
    custom_pl_chart->setInteraction(QCP::iRangeDrag, true);

    connect(this, &MainWindow::get_1000_ghz, this, &MainWindow::show_chart);
    connect(ui->pb_start, QPushButton::clicked, this, &MainWindow::clear_graph);


    //toDO


    //настроить лей оут (чтобы всместе растягивалось  с окном)
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
                                                 ...
                                                 */
                                                QCPGraph* graph = new QCPGraph(custom_pl_chart->xAxis, custom_pl_chart->yAxis);


                                                for (int i = 0; i < res.size(); i++){

                                                    graph->addData(i*0.001, res[i]);
                                                    if(i == 1000){
                                                      emit(get_1000_ghz(graph));
                                                      break;
...
void MainWindow::show_chart(QCPGraph* graph){


    custom_pl_chart->resize(window->size());
    custom_pl_chart->rescaleAxes();
    custom_pl_chart->replot();
    custom_pl_chart->show();
    window->show();
}

void MainWindow::clear_graph(){
    custom_pl_chart->clearGraphs();
    custom_pl_chart->replot();
}
